...
for sigma_longitude in longitudes_to_test:
    ...
    for sigma_latitude in latitudes_to_test:
        ...
        probabilistic_forecast = hef.gen_probabilistic_forecast(ensemble_members = df_combined.to_numpy().T, event_threshold = event_threshold, ensemble_size = ensemble_size)
        ...
        BS_cal, BS_ref = cf.compute_brier_components(forecasts = probabilistic_forecast, observations = df_data, num_bins=10)

        chi_across_latitude.append(chi_sq)
        BS_across_latitude.append(brier_score)
        ROC_across_latitude.append(roc_score)
        CAL_across_latitude.append(BS_cal)
        REF_across_latitude.append(BS_ref)
        
        t2=time.time()
        print(f'sigma_latitude = {sigma_latitude} evaluated in {t2-t1:.2f} seconds')

    cf.save_chi_arr_by_longitude_to_file(chi_set_list=chi_across_latitude, era_key=era_key, sigma_longitude=sigma_longitude, lead_time=lead_time)
    cf.save_BS_by_longitude_to_file(BS_set_list=BS_across_latitude, era_key=era_key, sigma_longitude=sigma_longitude, lead_time=lead_time, threshold=event_threshold)
    cf.save_ROC_by_longitude_to_file(ROC_set_list=ROC_across_latitude, era_key=era_key, sigma_longitude=sigma_longitude, lead_time=lead_time, probability_threshold=probability_threshold)
    cf.save_REF_by_longitude_to_file(REF_set_list=REF_across_latitude, era_key=era_key, sigma_longitude=sigma_longitude, lead_time=lead_time, threshold = event_threshold)
    cf.save_CAL_by_longitude_to_file(CAL_set_list=CAL_across_latitude, era_key=era_key, sigma_longitude=sigma_longitude, lead_time=lead_time, threshold = event_threshold)
